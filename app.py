...
if role == "Admin":
    ...
    # --- Section hyperparamètres dynamiques ---
    if model_name == "SurpriseSVD (SVD)":
        n_factors = st.slider("Number of latent factors (n_factors)", 10, 300, 150)
    if model_name == "UserBased":
        k = st.slider("Number of neighbors (k)", 1, 100, 50)
        min_k = st.slider("Minimum neighbors (min_k)", 1, 20, 1)
        sim_name = st.selectbox("Similarity metric", ["jaccard", "msd", "cosine"])
        min_support = st.slider("Min support (min_support)", 1, 10, 2)
    ...
    if model_name == "SlopeOne":
        st.info("No hyperparameters for SlopeOne.")
    ...
    if model_name != "ContentBased":
        if st.button("Train model"):
            ...
            if model_name == "SurpriseSVD (SVD)":
                model = SurpriseSVD(n_factors=n_factors)
            elif model_name == "UserBased":
                model = UserBased(
                    k=k,
                    min_k=min_k,
                    sim_options={"name": sim_name, "min_support": min_support}
                )
